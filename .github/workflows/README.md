...
Runs on pull requests targeting `main`, pushes to `main`, and manually. Ninja builds the `unit_tests` target with `-j2 -l4` to limit memory pressure, Cargo is limited to two jobs, and `tests.sh -L unit -j4` runs the unit suite without running end-to-end tests.

The `unit_tests` target includes all unit-test binaries, including those invoked by shell and Python wrappers, and their transitive CCF dependencies. It avoids building unrelated sample applications, benchmarks, and tools; the normal default build target is unchanged. Diagnostic logs, JUnit results, and Ninja command timings are retained for seven days.
