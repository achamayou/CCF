...
def _load_shared_trace_validation() -> ModuleType:
    veil_dir = pathlib.Path(__file__).resolve().parents[1] / "veil"
    module_path = veil_dir / "trace_validation.py"
    module_name = "_ccf_veil_trace_validation"
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    if spec is None or spec.loader is None:
        raise LeanTraceValidationError(
            f"cannot load shared trace planner from {module_path}"
        )

    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    sys.path.insert(0, str(veil_dir))
    try:
        spec.loader.exec_module(module)
    finally:
        sys.path.remove(str(veil_dir))
    return module


SHARED = _load_shared_trace_validation()
TracePlan = SHARED.TracePlan
TraceValidationError = SHARED.TraceValidationError
...
def _render_action(
    ...
    if step.action in SHARED.REQUEST_ACTIONS:
    ...
    if step.action == SHARED.EXECUTE_ACTION:
        return (
            ".rwTxExecute "
...
def _render_actions(plan) -> str:
    ...
    for step in plan.steps:
        uses_history = step.kind == "trace" and step.action != SHARED.EXECUTE_ACTION
...
def render_trace_source(plan) -> str:
    """Render a kernel-checked replay proof for a shared trace plan."""
...
abbrev initial : ConcreteState Tx View Seqno Event := ConcreteState.initial
...
theorem implementationTraceReachable :
    Exists fun final =>
      replay actions initial = some final /\\ Reachable final.toState :=
  replay_from_initial replaySucceeded

theorem implementationTraceProved :
    Exists fun final =>
      replay actions initial = some final /\\ ProvedBundle final.toState := by
  cases implementationTraceReachable with
  | intro final result =>
      exact
        Exists.intro final
          (And.intro result.1 (reachableProved result.2))

#print axioms implementationTraceProved

run_cmd do
  let axioms <- Lean.collectAxioms ``implementationTraceProved
  if axioms.contains ``sorryAx then
    throwError "generated trace proof depends on sorryAx"
  if axioms.contains ``Lean.ofReduceBool then
    throwError "generated trace proof depends on native compiler evaluation"
  Lean.logInfo m!"Audited generated pure Lean trace proof axioms."
...
def _write_plan(plan, output: TextIO) -> None:
    SHARED._write_plan(plan, output)
...
def main() -> int:
    ...
    try:
        with args.trace.open(encoding="utf-8") as trace_file:
            plan = SHARED.plan_trace(SHARED.parse_trace(trace_file))
