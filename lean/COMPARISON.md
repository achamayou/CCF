...
The Lean development translates the modules `ExternalHistory.tla`,
...
protocol in `tla/consensus`. The Lean model is a single level: the refinement
structure of the TLA+ tower is flattened into one transition system.
...
| Trace validation          |    156 |     1,265 |  8.1x |
| **Total**                 |**845** | **9,602** |**11.4x** |
...
| `trace_planner.py` (parser and planner)  |  429 |
| `trace_validation.py` (renderer)         |  280 |
| `test_trace_validation.py`               |  154 |
| **Total**                                |**1,265**|

TLA+ is 8.1 times smaller here, and the reason is architectural rather than
incidental.
...
where. That is what `trace_planner.py` does: it parses the NDJSON, normalises
ranks, backfills unlogged ledger entries, and reconstructs view changes, then
hands an exact list of steps to `trace_validation.py` to render as Lean. Those
429 lines are the price of not having a search, and they are ordinary Python
outside the proof, so they are trusted rather than checked.

So the comparison is really:

- TLA+: 156 lines and no external tooling, because the model checker can search.
- Lean: 402 lines of checked Lean plus 863 lines of trusted Python, because a
  proof assistant replaying a fixed sequence cannot.
...
It is worth being precise about what the planner can and cannot get wrong. If
it reconstructs the wrong internal actions, replay fails: the generated guard
will not hold and no proof is produced. It cannot cause a bad trace to be
accepted, because every accepted step still has to satisfy the model's own
guard. What it could do is reject a trace the model would have admitted, which
is a false alarm rather than a missed defect.
...
- **Nothing is implicit.** Every definition has a type; every use is checked.
  ...
  concrete instance: three definitions there are false or malformed as written,
  one of them vacuous, and they are recorded under "Corrections applied to the
  TLA+ source" in `README.md`. Those survived model checking. A vacuous clause
  in Lean is still possible, which is exactly why `Examples.lean` constructs a
  concrete request/execute/response path, but the type system rules out the
  whole class of arity and field errors.
...
Not attributed to either column above, for fairness. The Lean trace planner is
excluded from this table because it is counted in section 3, where it belongs.
...
Both approaches consume the same implementation trace artifact, produced by the
`consistency_trace_validation` test, so trace generation is not attributed to
either side.
...
5. **Trace validation strongly favours TLA+ on cost**, 156 lines against 1,265,
   because TLC can search for unlogged internal actions while a replay-based
   checker must be told what they were. Two thirds of the Lean figure is
   trusted Python outside the proof. The compensation is that the output is a
   kernel-checked theorem, and that a planner error can only reject a good
   trace, never accept a bad one.
